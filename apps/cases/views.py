...
def sanitize_search_snippet(text, max_length=160):
    """
    Sanitizes search result snippet text and truncates to max_length cleanly.
    """
    if not text:
        return ""
    import re
...
class GlobalSearchView(LoginRequiredMixin, View):
    """
    Global search view across accessible cases for documents, suspects, evidence, and notes.
    Ensures document search result snippets display sanitized text truncated to 160 characters.
    """
    def get(self, request):
        ...
        if query:
            ...
            sanitized_docs = []
            for doc in docs:
                latest_ai = doc.ai_results.order_by('-created_at').first()
                raw_summary = latest_ai.summary if latest_ai else ""
                if not raw_summary and hasattr(doc, 'extracted_text') and doc.extracted_text:
                    raw_summary = doc.extracted_text.raw_text
                doc.sanitized_snippet = sanitize_search_snippet(raw_summary, max_length=160)
                sanitized_docs.append(doc)

            results['documents'] = sanitized_docs
        ...
        return render(request, 'search/search_results.html', {
            ...
            'results': results,
            'all_cases': all_cases,
            'selected_case': int(case_filter) if case_filter and case_filter.isdigit() else None
        })
